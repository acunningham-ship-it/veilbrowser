...
export class Browser {
  ...
  static async connect(endpoint: string, opts: { blockPrivateNetwork?: boolean; allowOrigins?: string[]; fingerprint?: Fingerprint } = {}): Promise<Browser> {
    ...
    if (!/^wss?:\/\//i.test(endpoint)) {
      ...
      if (!res.ok) {
        throw new Error(
          `Browser.connect: ${origin}/json/version returned ${res.status}. ` +
            `Is Chrome running with --remote-debugging-port, and is the port right?`,
        );
      }
      const info: any = await res.json();
      wsUrl = info.webSocketDebuggerUrl;
      if (!wsUrl) throw new Error(`Browser.connect: ${origin} gave no webSocketDebuggerUrl.`);
    }
  ...
  async pages(): Promise<Page[]> {
    ...
    const wanted = (targetInfos || []).filter(
      (t: any) => t.type === "page" && !/^(devtools|chrome-extension|chrome):\/\//i.test(t.url || ""),
    );
    const out: Page[] = [];
    for (const t of wanted) {
      const { sessionId } = await this.cdp.send<any>("Target.attachToTarget", {
        targetId: t.targetId,
        flatten: true,
      });
