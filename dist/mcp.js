...
async function connectOrLaunch() {
    ...
    if (endpoint) {
        try {
            ...
            process.stderr.write(`[veil-mcp] no Chrome at ${endpoint} (${String(e).slice(0, 120)}) — launching one instead\n`);
        }
    }
    try {
...
async function callTool(name, args) {
    ...
    switch (name) {
        ...
        case "veil_double_click":
            if (args.ref != null)
                await p.doubleClick(args.ref);
            else if (args.x != null && args.y != null)
                await p.doubleClickAt(args.x, args.y);
            else
                throw new Error("veil_double_click: pass either ref, or both x and y");
            return text(args.ref != null ? `double-clicked [${args.ref}]` : `double-clicked (${args.x}, ${args.y})`);
        case "veil_right_click":
            if (args.ref != null)
                await p.rightClick(args.ref);
            else if (args.x != null && args.y != null)
                await p.rightClickAt(args.x, args.y);
            else
                throw new Error("veil_right_click: pass either ref, or both x and y");
        ...
        case "veil_drag":
            if (args.via?.length) {
                if (args.ref != null)
                    await p.dragRefToPath(args.ref, [...args.via, { x: args.toX, y: args.toY }]);
                else
                    await p.dragAtPath([{ x: args.fromX, y: args.fromY }, ...args.via, { x: args.toX, y: args.toY }]);
            }
